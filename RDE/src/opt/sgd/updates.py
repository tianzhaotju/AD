...
def create_update(nnet):
    ...
    if Cfg.softmax_loss:
        ...
        compile_update_deep_GMM(nnet, inputs, targets)
    else:
...
def compile_update_deep_GMM(nnet, inputs, targets):
    """
    create a deep GMM loss for network given in argument
    """
    ...
    category_layer = nnet.category_layer
    prediction = lasagne.layers.get_output(final_layer, inputs=inputs, deterministic=False)
    z_var = lasagne.layers.get_output(feature_layer, inputs=inputs, deterministic=False)
    gama = lasagne.layers.get_output(category_layer, inputs=inputs, deterministic=False)

    # use l2 or binary crossentropy loss (features are scaled to [0,1])
    if Cfg.ae_loss == "l2":
        resconstruct_loss = lasagne.objectives.squared_error(prediction, inputs)
    if Cfg.ae_loss == "ce":
        resconstructa_loss = lasagne.objectives.binary_crossentropy(prediction, inputs)

    resconstruct_loss = T.sum(resconstruct_loss, axis=range(1, ndim), dtype='floatX')
    ...
    trainable_params = lasagne.layers.get_all_params([final_layer, category_layer], trainable=True)



    mu_N_k_z, updates_mu = theano.scan(lambda z_s, gama_s: T.outer(z_s, gama_s), sequences=[z_var, gama])
    ...
    sigma_N_k_z_z, updates_sigma = theano.scan(
        lambda z_v, gama_v: theano.scan(lambda mu_v, gama_v_v: gama_v_v * T.outer(z_v - mu_v, z_v - mu_v), \
    ...
    logits, updates_logit = theano.scan(lambda z_v: theano.scan(lambda mu_v, sigma_v, phi_v: phi_v * T.exp(
        T.sum((-0.5) * T.dot((z_v - mu_v), T.nlinalg.matrix_inverse(sigma_v)) * (z_v - mu_v))) / T.sqrt(
        T.nlinalg.det(2 * np.pi * sigma_v)), sequences=[mu_k_z, sigma_k_z_z, phi_k]), sequences=z_var)

    logits = T.sum(logits, axis=1)

    #----------???modify------------------------------

    # logits = T.sum(z_var, axis=1) + T.sum(gama, axis= 1)

    #-----------------------------------------

    loss_prob = T.log(logits)



    # Network weight decay

    GMM_loss = T.mean(-loss_prob + resconstruct_loss)

    loss = T.cast(floatX(1) * (l2_penalty + GMM_loss), dtype='floatX')
    test_scores = T.cast(-loss_prob + resconstruct_loss + T.log(floatX(0.02)),dtype = 'floatX')

    score_single = T.cast(-loss_prob+ resconstruct_loss ,dtype = 'floatX')

    test_stack = T.stack([T.zeros_like(test_scores), test_scores], axis=1)
    test_loss = T.cast(T.sum(T.max(test_stack, axis=1)) / (inputs.shape[0]), dtype='floatX')
    ...
    test_acc = T.cast((T.sum(T.eq(test_y_pred.flatten(), targets), dtype='int32') * 1. / targets.shape[0]),
                      dtype='floatX')
    updates_GMM = get_updates(nnet, loss, trainable_params, solver=nnet.solver)
    print 'Start backpropagation'
    nnet.backprop_deep_GMM = theano.function([inputs, targets], [logits, resconstruct_loss , test_acc, mu_k_z, sigma_k_z_z],
                                             updates=updates_GMM,
                                             on_unused_input='warn')
    print 'Start forwarding'
    nnet.forward = theano.function([inputs, targets],
                                   [loss, test_acc, test_scores,score_single,test_y_pred, test_loss, loss_prob, resconstruct_loss, l2_penalty],
                                   on_unused_input='warn')
...
#____________________----------------------------------------------------
    ...
    mu_k_z_constant = theano.shared(floatX(np.ones(k,z)))

    sigma_k_z_z_constant = theano.shared(floatX(np.ones(k,z,z)))



    nnet.update_mu = theano.function([mu_k_z], updates=[(mu_k_z_constant, T.cast(floatX(0.1)*mu_k_z +floatX(0.9)*mu_k_z_constant, dtype= 'floatX'))],
                                             on_unused_input='warn')
    nnet.update_sigma = theano.function([sigma_k_z_z], updates=[(sigma_k_z_z_constant, T.cast(floatX(0.1)*sigma_k_z_z +floatX(0.9)*sigma_k_z_z_constant, dtype= 'floatX'))],
                                             on_unused_input='warn')

    logits_constant, updates_logit = theano.scan(lambda z_v: theano.scan(lambda mu_v, sigma_v, phi_v: phi_v * T.exp(
        T.sum((-0.5) * T.dot((z_v - mu_v), T.nlinalg.matrix_inverse(sigma_v)) * (z_v - mu_v))) / T.sqrt(
        T.nlinalg.det(2 * np.pi * sigma_v)), sequences=[mu_k_z_constant, sigma_k_z_z_constant, phi_k]), sequences=z_var)

    logits_constant = T.sum(logits_constant, axis=1)

    loss_prob_constant = T.log(logits_constant)

    GMM_loss_constant = T.mean(-loss_prob_constant + resconstruct_loss)

    loss_constant = T.cast(floatX(1) * (l2_penalty + GMM_loss_constant), dtype='floatX')

    test_scores_constant = T.cast(-loss_prob_constant + resconstruct_loss + T.log(floatX(0.02)), dtype='floatX')

    score_single_constant = T.cast(-loss_prob_constant + resconstruct_loss, dtype='floatX')

    test_stack_constant = T.stack([T.zeros_like(test_scores_constant), test_scores_constant], axis=1)
    test_loss_constant = T.cast(T.sum(T.max(test_stack_constant, axis=1)) / (inputs.shape[0]), dtype='floatX')

    test_y_pred_constant = T.argmax(test_stack_constant, axis=1)
    test_acc_constant = T.cast((T.sum(T.eq(test_y_pred_constant.flatten(), targets), dtype='int32') * 1. / targets.shape[0]),
                      dtype='floatX')

    print 'Start forwarding'
    nnet.forward_test = theano.function([inputs, targets],
                                   [loss_constant, test_acc_constant, test_scores_constant, score_single_constant, test_y_pred_constant, test_loss_constant, loss_prob_constant,
                                    resconstruct_loss, l2_penalty],
                                   on_unused_input='warn')
...
def compile_update_svdd(nnet, inputs, targets):
    ...
    rep_dim = feature_layer.num_units
    # maximum likehood
    volume =T.cast(floatX(-0.5)*T.sum(((rep) ** 2), axis=1, dtype='floatX'), dtype='floatX')
    ...
    trainable_params = lasagne.layers.get_all_params(final_layer, trainable=True)
    #Deep Gaussian Model
    updates_deep_kde = get_updates(nnet, deep_kde_loss, trainable_params, solver=nnet.solver)
