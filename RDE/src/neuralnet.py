...
class NeuralNet:
    ...
    def load_data(self, data_loader=None, pretrain=False):
        ...
        elif Cfg.deep_GMM_loss and Cfg.reconstruction_penalty:
            self.data.build_autoencoder(self)
    ...
    def adddiscriminator(self,num_1,num_2):
        ...
        #setattr(self, "category_layer", category)
    ...
    def save_diagnostics(self, which_set, epoch, scores, rep_norm, rep, emp_loss, reconstruction_penalty):
        ...
        if self.data.n_classes == 2:
            ...
            if sum(y) > 0:
                ...
                print ("{:32} {:.2f}%".format(which_set.title() + ' loss:', 100. * rep_norm))
                print ("{:32} {:.2f}%".format(which_set.title() + ' Maximal_log:', 1.*rep))
                print ("{:32} {:.2f}%".format(which_set.title() + ' Entropy:', 1.*emp_loss))
    ...
    def save_diagnostics_GMM(self, which_set, epoch, scores,score_single, pre_label, objective, log_pro, reconstruction, weight_penalty, accuracy, emp_loss):
        """
        save diagnostics for which_set of epoch
        """
